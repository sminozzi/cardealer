...
class cardealer_RecentWidget extends WP_Widget {
    ...
    $output = '<div class="CarDealer-listing-wrap"> <div class="carGallery">';
	...
	while ($_query3->have_posts()) : $_query3->the_post();
        ...
        $miles = get_post_meta(get_the_ID(), 'car-miles', true);
            ...
            $output .= ($miles <> '' ? __('Miles', 'cardealer') .': '. $miles.'<br />' : '');



            $output .= '</div>';
            $output .= '<div class="carTitle-widget">' . get_the_title() . '</div>';
...
class cardealer_featuredWidget extends WP_Widget {
    ...
    public function __construct() {
        ...
        'FeaturedWidget',         
        'Featured cars',                
        array( 'description' => __('A list of Featured cars', 'cardealer'), ) 
        );
    } 
    ...
    ?>
		...
		$image_id = get_post_thumbnail_id();
		$image_url = wp_get_attachment_image_src($image_id,'medium', true);	
		$price = number_format_i18n(get_post_meta(get_the_ID(), 'car-price', true),0);
		$image = str_replace("-".$image_url[1]."x".$image_url[2], "", $image_url[0]);
